...
export function sendBadgeLabel(summary) {
  if (!summary) return "等待预览";
  return summary.can_send === true ? "可确认发送" : "不可发送";
}

export function summaryAllowsConfirmSend(summary, nextActions) {
  return Boolean(
    summary?.can_send === true &&
      Array.isArray(nextActions) &&
      nextActions.includes("confirm_send"),
  );
}
