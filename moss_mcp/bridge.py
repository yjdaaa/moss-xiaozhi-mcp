...
async def connect_to_server(uri):
    ...
    try:
        ...
        async with websockets.connect(uri) as websocket:
            ...
            logger.info(f"Started {mcp_target} process")
        ...
        if 'process' in locals():
            logger.info(f"Terminating {mcp_target} process")
            try:
                process.terminate()
                process.wait(timeout=5)
            except subprocess.TimeoutExpired:
                process.kill()
            logger.info(f"{mcp_target} process terminated")
...
def main(argv=None):
    ...
    mcp_target = args[0] if args else "moss_mcp.server"
